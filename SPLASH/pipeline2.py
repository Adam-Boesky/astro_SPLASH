...
class Splash_Pipeline:
    ...
    def __init__(self, pre_transformed: bool = False, within_4sigma: bool = True, nan_thresh_ratio: int = 0.5, random_seed: int = 22):
        ...
        self.property_predicting_net = get_model(num_inputs=nn_hyperparams['num_inputs'], num_outputs=nn_hyperparams['num_outputs'], nodes_per_layer=nn_hyperparams['nodes_per_layer'], num_linear_output_layers=nn_hyperparams['num_linear_output_layers'])
    ...
    def infer_classes(
        ...
        """
        # Get the host props
        host_props_norm, _ = self.predict_host_properties(
            grizy,
            redshift,
            grizy_err,
            redshift_err,
        ...
        host_props = np.hstack((np.log10(angular_sep.reshape(-1, 1)), self.host_props, redshift))
    ...
    def get_transient_catalog(
        ...
        if self.transient_catalog is not None:
        ...
        transient_catalog = pd.DataFrame(data={
            'name': names if names is not None else [f'SN{i}' for i in range(len(ra))],  # arbitrary names
            'RA': ra,
            'DEC': dec,
        })
    ...
    def get_redshifts(self, ra: np.ndarray, dec: np.ndarray, progress_bar: bool = True, verbose: int = 1) -> np.ndarray:
        """Predict the redshifts of the given galaxies using the astro_prost package.

        Args:
            ra (np.ndarray): Right ascensions of the galaxies.
            dec (np.ndarray): Declinations of the galaxies.
        Returns:
            The redshifts of the given galaxies.
        """
        return self.get_transient_catalog(ra, dec, progress_bar=progress_bar, verbose=verbose)['host_redshift']

    def get_angular_seps(self, ra: np.ndarray, dec: np.ndarray, progress_bar: bool = True, verbose: int = 1) -> np.ndarray:
        """Predict the redshifts of the given galaxies using the astro_prost package.
        ...
        Returns:
            The redshifts of the given galaxies.
        """
        # Get the transient catalog
        cat = self.get_transient_catalog(ra, dec, progress_bar=progress_bar, verbose=verbose)['host_redshift']

        # Get the angular separations
        sn_coords = SkyCoord(ra=cat['RA'].values * u.deg, dec=cat['DEC'].values * u.deg)
        host_coords = SkyCoord(ra=cat['host_ra'].values * u.deg, dec=cat['host_dec'].values * u.deg)
        separations = sn_coords.separation(host_coords)
