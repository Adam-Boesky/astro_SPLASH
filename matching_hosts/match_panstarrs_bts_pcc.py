...
# os.mkdir = 'ps1_dir'
OUTPUT_FILENAME = 'bts_hosts.ecsv'
...
def get_current_data():
    ...
    # Grab the sne data
    bts_sne = pd.read_csv(os.path.join(PATH_TO_STORAGE, 'bts_sne_clean.csv'))
    ...
    if os.path.exists(os.path.join(PATH_TO_STORAGE, OUTPUT_FILENAME)):
        ...
        for i, sn_ra, sn_dec in zip(range(n), bts_sne['ra'], bts_sne['dec']):
    ...
    return bts_sne, last_searched_ind, col_types, all_res
...
def match_host_sne():
    ...
    for i, sn_ra, sn_dec, sn_z, sn_class, ztf_id in zip(range(n)[last_searched_ind + 1:], sne['ra_deg'][last_searched_ind + 1:], sne['dec_deg'][last_searched_ind + 1:], sne['redshift'][last_searched_ind + 1:], sne['type'][last_searched_ind + 1:], sne['ZTFID'][last_searched_ind + 1:]):
        if isinstance(sn_class, str) and sn_class != 'Candidate':  # only do the search if the SN is classified!!!

            # Convert SN coords to degrees  (artifact of copying code, sorry for bad practice :)  )
            sn_ra_ang = sn_ra
            sn_dec_ang = sn_dec
